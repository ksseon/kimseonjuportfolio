// HomeVisual.jsx — 인트로(단독 표시 + 스크롤 잠금) → 히어로(스크롤 가능) 전환
...
/* ===== 타이밍 설정 ===== */
const INTRO_DRAW_DURATION = 8.0; // 인트로 라인 그리기 시간
const INTRO_FADE_DURATION = 0.35; // 인트로 라인 사라지는 시간
const INTRO_WRAP_FADE = 0.28; // 인트로 전체 페이드아웃 시간
...
const HomeVisual = () => {
  ...
  /* 전역 클래스 관리(스크롤 잠금은 CSS에서 html.intro-active로 처리) */
  useEffect(() => {
  ...
  return (
    <section ref={wrapRef} className="intro-hero">
      {/* 비디오 배경 */}
      <video
      ...
      >
        ...
        {/* 오른쪽 상단 곡선 — 화면 밖(오른쪽)에서 시작하도록 살짝 밀어냄 */}
        <svg
        ...
        <div className="hero__grid">
          ...
          <div className="hero__right">
            <p className="hero__desc">
              배의 주인, 선주는 혼자 앞서가는 사람이 아니라, 바람을 읽고
              모두가안전하게 나아가도록 돛과 노를 함께 맞추는 조율자입니다.
              이처럼 저도 팀원 및 동료들과 협력하여 각자의 강점을 모아 하나의 큰
              바다를 향해 나아가겠습니다. 또한 끊임없이 밀려오는 파도 속에서도
              저 역시 도전을 두려워하지 않고, 책임감을 가지고 직관적이고 편리한
              사용자 여정으로 완성하는 개발자가 되고자 합니다.
            </p>
          </div>
        </div>
      </div>
